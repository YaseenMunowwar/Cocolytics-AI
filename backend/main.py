...
# Enable CORS middleware (restrict origins as required)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # For production, limit these origins appropriately.
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Set your OpenAI API key—replace with your actual key or load from an environment variable.
openai.api_key = "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"
...
def forecast():
    ...
    # Get the actual current month string (e.g., "April 2025")
    current_month_str = datetime.now().strftime("%B %Y")
    
    # Updated prompt: instruct the model to produce a neatly formatted summary with clear vertical spaces.
    summary_prompt = (
        "Please produce a professional and attractive summary of the coconut price forecast data with the following structure:\n\n"
        "1. Current Month:\n"
        "   Display the current retail price (per nut) for " + current_month_str + " on its own line.\n\n"
        "2. Coconut Price Trend:\n"
        "   Describe the overall trend from the beginning of the forecast period to " + current_month_str + ", emphasizing the steady increase.\n\n"
        "3. Forecast Overview:\n"
        "   List each monthly forecast price (one line per month).\n\n"
        "4. Actionable Recommendations:\n"
        "   Provide distinct, actionable recommendations for Producers, Investors, and Supply Chain Managers. Each recommendation should be on its own line and start with a symbol such as ✓ or ↑.\n\n"
        "5. Conclusion:\n"
        "   Summarize the key takeaways in one or two distinct bullet points.\n\n"
        "Use symbols (e.g., ✓, ↑, •) and use HTML <b> tags for bold text as needed. Do not use markdown hashtags.\n\n"
        "Forecast Data: " + str(forecast_results)
    )
